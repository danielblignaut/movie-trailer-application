...
'''the PATH global variabbles are used to help locate the data source file & the client angular app'''
...
''' Initiate Flask & fetch the list of movies. Also ensure that the server allows cross origin domain requests so that
our client can get JSON data from the server (it\'s hosted on a seperate server. '''

app = Flask(__name__, static_url_path=CLIENT);
Center = entertainment_center.Entertainment_Center(DATABASE);

'''route a GET request through the API to return the number of movies in the list '''
